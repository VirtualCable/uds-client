...
use tiny_skia::{Color, LinearGradient, Paint, PathBuilder, Pixmap, SpreadMode, Stroke, Transform, GradientStop, Point};
    ...
    for (idx, wave) in waves.iter().enumerate() {
        ...
        if let Some(path) = pb.finish() {
            ...
            let grad = LinearGradient::new(
                ...
                vec![
                    GradientStop::new(0.0, Color::from_rgba8(main_color[0], main_color[1], main_color[2], 0)),
                    GradientStop::new(0.5, Color::from_rgba8(main_color[0], main_color[1], main_color[2], (wave.opacity * 255.0) as u8)),
                    GradientStop::new(1.0, Color::from_rgba8(main_color[0], main_color[1], main_color[2], 0)),
                ],
                SpreadMode::Pad,
                Transform::identity(),
            ).unwrap();
            stroke_paint.shader = grad;
            let stroke = Stroke { width: wave.thickness * scale, ..Default::default() };
