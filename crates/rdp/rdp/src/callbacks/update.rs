...
pub trait UpdateCallbacks {
    ...
    /// # Safety
    /// The `bounds` pointer must be valid.
    ...
    fn on_bitmap_update(&mut self, bitmap: *const BITMAP_UPDATE) -> bool {
        if bitmap.is_null() {
            log::debug!("Bitmap update called with null pointer");
            return false;
        }
        true
    }
    ...
    fn on_palette(&mut self, palette: *const freerdp_sys::PALETTE_UPDATE) -> bool {
        if palette.is_null() {
            log::debug!("Palette update called with null pointer");
            return false;
        }
        true
    }
    ...
    fn on_play_sound(&mut self, play_sound: *const freerdp_sys::PLAY_SOUND_UPDATE) -> bool {
        if play_sound.is_null() {
            log::debug!("Play sound update called with null pointer");
            return false;
        }
        true
    }
    ...
    fn on_set_keyboard_indicators(&mut self, led_flags: u16) -> bool {
        log::debug!("Set keyboard indicators: led_flags={}", led_flags);
        true
    }
    ...
    fn on_refresh_rect(&mut self, count: u8, areas: *const RECTANGLE_16) -> bool {
        if areas.is_null() {
            log::debug!("Refresh rect called with null pointer");
            return false;
        }
        log::debug!("Refresh rect: count={}", count);
        true
    }

    fn on_suppress_output(&mut self, allow: u8, area: *const RECTANGLE_16) -> bool {
        if area.is_null() {
            log::debug!("Suppress output called with null pointer");
            return false;
        }
        log::debug!("Suppress output: allow={}", allow);
        true
    }
    ...
    fn on_remote_monitors(
        ...
        if monitors.is_null() {
            log::debug!("Remote monitors called with null pointer");
            return false;
        }
        log::debug!("Remote monitors: count={}", count);
        true
    }

    fn on_surface_command(&mut self, s: *mut freerdp_sys::wStream) -> bool {
        if s.is_null() {
            log::debug!("Surface command called with null pointer");
            return false;
        }
        true
    }
    ...
    fn on_surface_bits(&mut self, surface_bits: *const freerdp_sys::SURFACE_BITS_COMMAND) -> bool {
        if surface_bits.is_null() {
            log::debug!("Surface bits called with null pointer");
            return false;
        }
        true
    }
    ...
    fn on_surface_frame_marker(
        ...
        if surface_frame_marker.is_null() {
            log::debug!("Surface frame marker called with null pointer");
            return false;
        }
        true
    }
    ...
    fn on_surface_frame_bits(
        ...
        if cmd.is_null() {
            log::debug!("Surface frame bits called with null pointer");
            return false;
        }
        log::debug!(
    ...
    fn on_surface_frame_acknowledge(&mut self, frame_id: u32) -> bool {
        log::debug!("Surface frame acknowledge: frame_id={}", frame_id);
        true
    }

    fn on_save_session_info(&mut self, type_: u32, data: *mut ::std::os::raw::c_void) -> bool {
        if data.is_null() {
            log::debug!("Save session info called with null pointer");
            return false;
        }
        log::debug!("Save session info: type={}", type_);
        true
    }

    fn on_server_status_info(&mut self, status: u32) -> bool {
        log::debug!("Server status info: status={}", status);
        true
    }
}
