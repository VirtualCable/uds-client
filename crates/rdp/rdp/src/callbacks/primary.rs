...
pub trait PrimaryCallbacks {
    fn on_dst_blt(&self, dstblt: *const DSTBLT_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_dst_blt called: dstblt={:?}", dstblt);
        true
    }

    fn on_pat_blt(&self, patblt: *mut PATBLT_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_pat_blt called: patblt={:?}", patblt);
        true
    }

    fn on_scr_blt(&self, scrblt: *const SCRBLT_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_scr_blt called: scrblt={:?}", scrblt);
        true
    }
    ...
    fn on_line_to(&self, line_to: *const LINE_TO_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_line_to called: line_to={:?}", line_to);
        true
    }

    fn on_polyline(&self, polyline: *const POLYLINE_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_polyline called: polyline={:?}", polyline);
        true
    }

    fn on_mem_blt(&self, memblt: *mut MEMBLT_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_mem_blt called: memblt={:?}", memblt);
        true
    }

    fn on_mem3_blt(&self, mem3blt: *mut MEM3BLT_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_mem3_blt called: mem3blt={:?}", mem3blt);
        true
    }
    ...
    fn on_glyph_index(&self, glyph: *mut GLYPH_INDEX_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_glyph_index called: glyph={:?}", glyph);
        true
    }

    fn on_fast_index(&self, glyph: *const FAST_INDEX_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_fast_index called: glyph={:?}", glyph);
        true
    }

    fn on_fast_glyph(&self, glyph: *const FAST_GLYPH_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_fast_glyph called: glyph={:?}", glyph);
        true
    }

    fn on_polygon_sc(&self, polygon_sc: *const POLYGON_SC_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_polygon_sc called: polygon_sc={:?}", polygon_sc);
        true
    }

    fn on_polygon_cb(&self, polygon_cb: *mut POLYGON_CB_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_polygon_cb called: polygon_cb={:?}", polygon_cb);
        true
    }

    fn on_ellipse_sc(&self, ellipse_sc: *const ELLIPSE_SC_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_ellipse_sc called: ellipse_sc={:?}", ellipse_sc);
        true
    }

    fn on_ellipse_cb(&self, ellipse_cb: *const ELLIPSE_CB_ORDER) -> bool {
        log::debug!(" 🧪 **** Default on_ellipse_cb called: ellipse_cb={:?}", ellipse_cb);
        true
    }
