...
mod tests {
    ...
    fn parse_rsa_components_roundtrip() {
        ...
        let mut rng = rsa::rand_core::OsRng;
        let key = rsa::RsaPrivateKey::new(&mut rng, 2048).unwrap();
        let (n, e, d) = parse_rsa_pkcs1_components(key.to_pkcs1_der().unwrap().as_bytes()).unwrap();
    ...
    fn select_wrong_aid() {
        let mut e = make_test_engine(PinMode::NotRequired);
        let apdu = vec![
            0x00, 0xA4, 0x04, 0x00, 0x05, 0x00, 0x01, 0x02, 0x03, 0x04, 0x00,
        ];
        let r = e.process_apdu(&apdu);
    ...
    fn verify_pin_correct() {
        ...
        let apdu = vec![
            0x80, 0xB1, 0x00, 0x80, 0x07, 0x74, 0x65, 0x73, 0x74, 0x70, 0x69, 0x6E,
        ];
        let r = e.process_apdu(&apdu);
        assert_sw(&r, SW_SUCCESS);
    ...
    fn get_public_key_format() {
        ...
        let exp_len = u16::from_be_bytes([full[0], full[1]]) as usize;
        assert_eq!(exp_len, 3);
        let exp_bytes = &full[2..2 + exp_len];
        assert_eq!(exp_bytes, &[0x01, 0x00, 0x01]);

        let mod_offset = 2 + exp_len;
        let mod_len = u16::from_be_bytes([full[mod_offset], full[mod_offset + 1]]) as usize;
    ...
    fn sign_with_pin() {
        ...
        let di = vec![
            0x30, 0x31, 0x30, 0x0D, 0x06, 0x09, 0x60, 0x86, 0x48, 0x31, 0x02, 0x01, 0x00, 0x00,
